...
const Product = React.memo(lazyProps => {
  ...
  const brand = get(state, 'pageData.brand') || {}
  ...
  // Fetch variant data upon changing brand options
  useDidMountEffect(() => {
    const query = qs.stringify({ brand: brand.id }, { addQueryPrefix: true })
          ...
          <Grid container spacing={4}>
            <Grid item xs={12} sm={6} md={5}>
              ...
              <MediaCarousel
                ...
                height="100%"
                media={brand.media || (product && product.media)}
              />
            </Grid>
                  ...
                  {product ? (
                    <>
                      <Hbox style={{ marginBottom: 10 }}>
                        <Label>Brand: </Label>
                        <Typography>{brand.text}</Typography>
                      </Hbox>
                      <ProductOptionSelector
                        options={product.brands}
                        value={brand}
                        onChange={value =>
                          updateState({ ...state, pageData: { ...state.pageData, brand: value } })
                        }
